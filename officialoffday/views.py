...
def addoffday(request):
    ...
    response_data, response_message, response_successflag, response_status = ghelp().addtocolass(MODELS_OFFI.Offday, SRLZER_OFFI.Offdayserializer, request.data, unique_fields=unique_fields, extra_fields=extra_fields)
...
def updateoffday(request, offdayid=None):
    ...
    allowed_fields = ['day', 'is_active']
    response_data, response_message, response_successflag, response_status = ghelp().updaterecord(MODELS_OFFI.Offday, SRLZER_OFFI.Offdayserializer, offdayid, request.data, allowed_fields=allowed_fields, extra_fields=extra_fields)
    return Response({'data': response_data, 'message': response_message, 'status': response_successflag}, status=response_status)